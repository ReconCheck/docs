...
> 边界（先说清楚）：支持**表格类文件（CSV/TSV/XLSX）与 PDF 文本层**；**扫描件（无文本层的 PDF）与图片**需 OCR，暂未支持。文件名里需要包含**相同的业务编号**才能自动配对（见 FAQ）。
...
**三方核对**：采购订单 + 送货单 + 发票三份一起核对时，可用 `reconcheck compare3` 或 `POST /api/compare3`（3 个文件或 3 个 `doc_ids`）——返回三对两两报告 + 「哪一方是偏离方」的判定（如 发票 vs 采购单+送货单），适合月底对账一票到底。

### 3. 提交并查看结果

- 点「运行」。每对会独立执行，进度逐对显示。
- 结果页：顶部是**高/中/低差异计数**；每对展开后列出每条差异：字段、左/右两侧的值、所在行/列、严重度。
...
**Q：为什么报「没有可配对的单据」？**
文件名剥离单据种类词（po/inv/采购/发票…）后的业务编号不一致，或同组不足两份。命名示例：`PO-240913-001` ↔ `INV-240913-001` 能配对；`a.csv`、`b.csv` 不能。
...
**Q：PDF 支持吗？**
支持有**文本层**的 PDF（导出/电子签章的单据）：线框表格优先，无框版面按坐标聚类兜底；**扫描件**（纯图片 PDF）会报「需 OCR」——OCR 后端尚未接入，后续里程碑。

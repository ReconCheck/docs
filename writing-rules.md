...
| 日期容差 | `when: dates_within: { days: 3 }` | 两侧都能解析为日期且差 ≤3 天 → 不报；**超过容差的真实日期差会照常报差异**（支持 ISO、斜杠、点号、紧凑、中文年月日格式） |
| 文本相等（忽略大小写） | `when: text_equal_ignore_case: true` | 两侧文本忽略大小写相等 → 不报；**真不一致（如 OK vs OPEN）会照常报差异** |

例外是**并列命中任一即放行**；可以在一条规则下配多个例外。日期/文本两类例外会让「非数值列」也能产出差异判定（见 §5 排查第 6 条）。
...
4. **例外命中**：单位换算、四舍五入、日期容差、忽略大小写后相等。测试时把例外临时清掉看是否恢复报错。
5. **证据缺失**：`evidence.require: both_sides` 下若某侧单元格解析不到坐标（例如合并单元格），该条被跳过。
6. **日期/文本差异不报**：非数值列默认不判定；只有规则声明了 `dates_within` / `text_equal_ignore_case` 且两侧格式可解析时，超出例外范围的差异才会报告。
